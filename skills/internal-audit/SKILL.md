...
- `gh` must be authenticated and the repo must have a GitHub remote (`gh repo view`). If `gh` isn't available or isn't authenticated, skip straight to printing the checklist markdown (see step 1.2) and continue the hands-on audit anyway - the issue can be created manually later.
...
2. Ask the user for confirmation before creating anything, using your interactive question tool: "Create a GitHub issue titled 'Internal audit' with the checklist below?" Options: create it now / don't create it, just show me the markdown.
